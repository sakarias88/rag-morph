...
function TOOL:IsValidRagdoll( rag, nrOfBones )
	for i = 0, nrOfBones-1 do
		if table.HasValue( self.boneTable , rag:GetBoneName( i ) ) then
			return true
		end
	end
...
function TOOL:LeftClick( trace )
	...
	if not self:CheckValidRagdoll( trace.Entity, trace.Entity:GetPhysicsObjectCount() ) then return false end

	self:GetOwner().defaultMorphModel = trace.Entity:GetModel()
	return true
end
